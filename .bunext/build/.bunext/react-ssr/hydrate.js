...
async function hydrate(Shell, {
  ...
  return import_client.hydrateRoot(document, /* @__PURE__ */ jsx_dev_runtime.jsxDEV(RouterHost, {
    Shell,
    ...options,
    children: /* @__PURE__ */ jsx_dev_runtime.jsxDEV(Shell, {
...
function Dev({ children }) {
  ...
  const [_ws, setWs] = import_react.useState();
  const resetWs = import_react.useCallback((setter) => {
    setter((ws) => {
      if (ws)
        ws.close();
      return;
  ...
  const MakeWebSocket = import_react.useCallback(() => {
    const p = window.location;
    try {
      const ws = new WebSocket(`${p.protocol.includes("https") ? "wss" : "ws"}://${p.hostname}:${globalThis.serverConfig.Dev.hotServerPort}`);
